...
class ControllerPaymentWesternUnion extends Controller {
	protected function index() {
		...
		$this->data['western_union'] = nl2br($this->config->get('western_union' . $this->config->get('config_language_id')));
	...
	public function confirm() {
		...
		$comment .= $this->config->get('western_union' . $this->config->get('config_language_id')) . "\n\n";
		$comment .= $this->language->get('text_payment');
		
		$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('western_union_order_status_id'), $comment, true);
	}
}
?>